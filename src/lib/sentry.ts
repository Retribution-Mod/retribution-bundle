...
function redactEvent(event: ErrorEvent): ErrorEvent | null {
    // Never send user/request data or per-user identifiers
    delete event.user;
    ...
    if (event.breadcrumbs) {
        for (const crumb of event.breadcrumbs) {
            // Drop network breadcrumbs completely to avoid leaking headers/tokens
